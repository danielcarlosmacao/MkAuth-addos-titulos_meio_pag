    ...
    <div class="container">
        ...
        <?php if ($acesso_permitido) : ?>
            <form id="searchForm" method="GET">
                <label for="search">Meio de pagamento:</label>
                ...
                <button  type="button" onclick="clearSearchMeio()" class="clear-button">Limpar Meio de Pamento</button>
            ...
            $query = "SELECT c.login, c.datavenc, c.datapag, c.valorpag, c.formapag , c.coletor, c.referencia
                      FROM sis_lanc c";
            ...
            // Se datas foram fornecidas no formulário de pesquisa, adicione condições à consulta SQL
                ...
                $query .= " WHERE DATE(c.datapag) BETWEEN '$data_inicial' AND '$data_final'";
            }
            ...
            if($search_term == true){
            $query .= " AND c.formapag LIKE '%$search_term%'";
            }}
           ...
           // echo($query)
            ...
            <?php if ($result && mysqli_num_rows($result) > 0) : ?>
                <table>
                    <thead>
                        <tr style="background:<?php echo($cor);?>" class="scroll-container">
                            ...
                            <th style="color: white;">Meio de Pagamento</th>
                            ...
                            <td style="text-align: left; cursor: default;"> 
                              ...
                              /*  preg_match('/titulo (\d+)/', $row['historico'], $matches);
                               ...
                               /**/ echo '<a href="../../cliente_det.hhvm?uuid=' . $uuid_cliente . '" target="_blank" style="color: #06683e; display: flex; align-items: center;" title="' . $nome_cliente . '">';
                                ...
                                <!-- Exibir LOGIN-->
                                <td style="font-weight: bold;"><?php echo $row['login']; ?></td>

                                <?php
                                
                                $dtpag = date('y-m-d', strtotime($row['datapag']));
                                $dtvenc= date('y-m-d', strtotime($row['datavenc']));

                                $diferenca = strtotime($dtpag) - strtotime($dtvenc);
                                ...
                                <td style="font-weight: bold; color:<?php if($dias > $variacaodias || $dias < -$variacaodias){echo "red";} ?>"><?php echo date('d-m-Y', strtotime($row['datavenc'])); ?></td> <!-- Data -->

                                <!--Exibe Data Pagamento--> 
                                <td style="font-weight: bold;"><?php echo date('d-m-Y', strtotime($row['datapag'])); ?></td> <!-- Data -->
                                ...
                                <td style="color:#0d6cea; font-weight: bold;">
                                   ...
                                   $link_busca = "?search=" . urlencode($row['formapag']) . "&data_inicial=" . urlencode($data_inicial) . "&data_final=" . urlencode($data_final);

                                   // Definir o comprimento máximo desejado para o nome de login
                                   $max_length = 15;

                                   // Limitar o tamanho do nome de login, se necessário
                                   $login_truncado = (strlen($row['formapag']) > $max_length) ? substr($row['formapag'], 0, $max_length) . '...' : $row['formapag'];
                                   ?>
                                   <a href="<?php echo $link_busca; ?>" title="<?php echo htmlspecialchars($row['formapag']); ?>" style="text-decoration: none; color: inherit;"><?php echo $login_truncado; ?></a>
                                </td>

                                <!--Exibe Valor pago -->
                                <td style="color: green; font-weight: bold;"><?php echo $row['valorpag']; ?></td>

                                <!--Exibe coletor -->
                                <td style="font-weight: bold;"><?php echo $row['coletor']; ?></td>
